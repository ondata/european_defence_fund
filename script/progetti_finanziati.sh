...
jq '.results[0:3] | map(.metadata.participants |= (if type == "array" then map(fromjson) else . end))' "$folder"/../data/progetti_finanziati/raw/progetti_finanziati.json > "$folder"/../data/sample/progetti_finanziati_sample.json

# Crea un file con solo i partecipanti e il riferimento al progetto
jq '.results[0:3] | map({reference: .reference, participants: (.metadata.participants | if type == "array" then map(fromjson) else . end)})' "$folder"/../data/progetti_finanziati/raw/progetti_finanziati.json > "$folder"/../data/sample/progetti_finanziati_participants_sample.json
